...
TEST_LABELS = {'2012': '2YP', '2013': '1YP', '2014': 'Y00'}
VERIFICATION_LABELS = {'2013': '2YP', '2014': '1YP', '2015': 'Y00'}
...
def main(filename, start_year):
    ...
    for x in dataframe.columns: 
        if 'EIN' not in x:
            year = x[:4]
            if str(start_year) = '2012':
                gen_year = TEST_LABELS[year]
            if str(start_year) = '2013'
                gen_year = VERIFICATION_LABELS[year]
            new_col_names.append(gen_year + x[4:])
    ...
    X_variables = [i for i in dataframe.columns if not i == y_variable]
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        print('Usage: -u model.py <datafilename> <start year>')
